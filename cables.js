...
export function createCables(root,{display,esp,battery,charger,strip,switches,shellFront,shellRear}){
 ...
 function update(current,dt){
  ...
  nets.forEach((net,index)=>{
   ...
   if(current<.8){
    const benchSink=160*smooth(current,.12,1.15);
    const r=net.ribbonIndex;
    if(r!==undefined){const y=96+(r-4.5)*.92-benchSink;a=V(6,y,1.05);b=V(-24.4,y,2.6);guide=new THREE.CatmullRomCurve3([a,V(-6,y,1.05),V(-18,y,1.05),V(-27.5,y,1.05),V(-30.7,y,4.2),V(-27.5,y,7.4),V(-24.6,y,4.6),b]);}
