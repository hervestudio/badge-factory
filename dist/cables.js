...
export function createCables(root,{display,esp,battery,charger,strip,switches}){
 ...
 const nets=[];
 const add=(name,color,a,b,purpose)=>nets.push({name,color,a,b,purpose});
 const colors=['#70422b','#d52b27','#ef791a','#e7c526','#229657','#246fc1','#8b43c5','#90979c','#eee8dc','#151621'];
 const displayPins=[['GND',s(2),'Common ground'],['VCC',e(-1,1,'3V3'),'Display power · 3.3 V'],['SCL',e(-1,20,'GPIO 14'),'SPI clock'],['SDA',e(-1,19,'GPIO 13'),'SPI MOSI data'],['RST',e(-1,18,'GPIO 12'),'Display reset'],['DC',e(-1,17,'GPIO 11'),'Data / command'],['CS',e(-1,16,'GPIO 10'),'SPI chip select'],['BL',e(-1,15,'GPIO 9'),'Backlight PWM'],null,['TE',e(-1,13,'GPIO 3'),'Tearing-effect synchronization']];
 displayPins.forEach((pin,i)=>{if(pin)add(`Display / ${pin[0]}`,colors[i],d(i,pin[0]),pin[1],pin[2]);});
 ...
 add('Sense / battery','#e88130',p(0,'B+'),s(0),'Battery feed to 100 kΩ divider');
 add('Sense / battery ADC','#b697df',s(1),e(1,4,'GPIO 1'),'Divided battery voltage · ADC');
 add('Sense / ground','#25262e',s(2,8.89),e(-1,22,'GND'),'Common bus to DevKit ground');
 add('Sense / charge ADC','#4cabc0',s(3),e(1,5,'GPIO 2'),'Divided USB input voltage · ADC');
 add('Sense / USB input','#e0ac36',anchor(charger,[8,-4,-1],'Charger · IN+ / VBUS'),s(4),'USB 5 V input to 100 kΩ divider');
 const group=new THREE.Group();root.add(group);let selected=-1;
 const N=25,R=6,delta=V(),tangent=V(),normal=V(),binormal=V(),up=V(0,0,1);
 ...
 function update(current,dt){
  ...
  nets.forEach((net,index)=>{
   const bench=1-smooth(current,.3,.8),reveal=smooth(current,6.9+index*.025,7.25+index*.025);net.shown=Math.max(bench,reveal,index===selected&&current>6.8?1:0);net.mesh.visible=net.shown>.001;if(!net.mesh.visible){net.started=false;return}
   let a=world(net.a),b=world(net.b),guide;
   if(current<.8){a=V(39+index*.75,108,3);b=V(39+index*.75,77,3);guide=new THREE.CatmullRomCurve3([a,V(a.x-3,103,3),V(b.x+3,82,3),b]);}
   else {
    // The central span folds into the side cavity as the two halves close.
    const lane=(index%9)*.6,mid=a.clone().lerp(b,.5);mid.z=THREE.MathUtils.lerp(24+lane,-7-lane*.25,closed);mid.y-=7+index%4;
   ...
   const targets=guide.getPoints(N-1),rest=targets.slice(1).map((v,i)=>v.distanceTo(targets[i])*1.015);
   ...
   for(let i=1;i<N-1;i++){const node=net.nodes[i],previous=node.clone();delta.copy(node).sub(net.old[i]).multiplyScalar(.87);node.add(delta).addScaledVector(targets[i].clone().sub(node),Math.min(1,step*12));node.y-=18*step*step;net.old[i].copy(previous);}
   for(let pass=0;pass<7;pass++){net.nodes[0].copy(a);net.nodes[N-1].copy(b);for(let i=0;i<N-1;i++){delta.copy(net.nodes[i+1]).sub(net.nodes[i]);const length=delta.length()||1;delta.multiplyScalar((length-rest[i])/length*.5);if(i>0)net.nodes[i].add(delta);if(i+1<N-1)net.nodes[i+1].sub(delta)}}
   net.nodes[0].copy(a);net.nodes[N-1].copy(b);
   const positions=net.mesh.geometry.attributes.position;
   for(let i=0;i<N;i++){tangent.copy(net.nodes[Math.min(N-1,i+1)]).sub(net.nodes[Math.max(0,i-1)]).normalize();normal.crossVectors(tangent,up);if(normal.lengthSq()<.01)normal.set(1,0,0);normal.normalize();binormal.crossVectors(tangent,normal);const radius=index===selected?.62:.34;for(let j=0;j<R;j++){const t=j/R*Math.PI*2,p=net.nodes[i];positions.setXYZ(i*R+j,p.x+radius*(normal.x*Math.cos(t)+binormal.x*Math.sin(t)),p.y+radius*(normal.y*Math.cos(t)+binormal.y*Math.sin(t)),p.z+radius*(normal.z*Math.cos(t)+binormal.z*Math.sin(t)))}}
   ...
   net.mesh.material.emissive.set(net.color);net.mesh.material.emissiveIntensity=index===selected?.5:0;
  });
 }
 function pick(event,camera,canvas){const rect=canvas.getBoundingClientRect();let best=12,index=-1;const pointer=new THREE.Vector2(event.clientX,event.clientY);nets.forEach((n,j)=>{if(n.shown<.95)return;for(let i=0;i<N-1;i++){const a=root.localToWorld(n.nodes[i].clone()).project(camera),b=root.localToWorld(n.nodes[i+1].clone()).project(camera),p=new THREE.Vector2(rect.left+(a.x+1)*rect.width/2,rect.top+(1-a.y)*rect.height/2),q=new THREE.Vector2(rect.left+(b.x+1)*rect.width/2,rect.top+(1-b.y)*rect.height/2),v=q.clone().sub(p),t=clamp(pointer.clone().sub(p).dot(v)/(v.lengthSq()||1),0,1),distance=pointer.distanceTo(p.addScaledVector(v,t));if(distance<best){best=distance;index=j}}});if(index>=0)choose(index);}
 return {update,pick,nets};
}
