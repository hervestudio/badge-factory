...
try {
 ...
 workbench.visible=current<1.12;cutting.material.opacity=1;top.material.opacity=1;workbench.position.set(0,-260*leaveBench,0);
 ...
 if(p.userData.benchOnly){p.visible=current<1.05;p.scale.setScalar(1);shellA.copy(bench);shellA.y-=260*leaveBench;shellA.z=bench.z+p.userData.lift;shellA.sub(p.parent.position).applyAxisAngle(YAXIS,-p.parent.rotation.y);p.position.copy(shellA);p.rotation.y=benchRotation-p.parent.rotation.y;continue;}
 ...
 if(isShell){const g=p.parent,tShell=name==='Front enclosure'?smooth(leaveBench,.3,1):smooth(leaveBench,0,.7);shellB.copy(destination).applyAxisAngle(YAXIS,g.rotation.y).add(g.position);shellA.copy(bench).lerp(shellB,tShell);const arcT=Math.sin(Math.PI*tShell);shellA.y+=42*arcT;shellA.z+=34*arcT;shellA.sub(g.position).applyAxisAngle(YAXIS,-g.rotation.y);p.position.copy(shellA);p.rotation.y=tShell>=1?0:lerp(benchRotation,name==='Front enclosure'?2.88:-.13,tShell)-g.rotation.y;}else if(entry>0){p.position.copy(destination);p.position.y-=55*(1-entry);}else{shellA.copy(bench);shellA.y-=260*leaveBench;shellA.sub(p.parent.position).applyAxisAngle(YAXIS,-p.parent.rotation.y);p.position.copy(shellA);}if(p.userData.lift>.01)p.position.z+=p.userData.lift*(1-leaveBench);if(!isShell){p.rotation.y=entry>0?lerp(benchRotation,0,entry):benchRotation-p.parent.rotation.y;p.rotation.x=name==='M2×12 screw'?lerp(Math.PI/2,0,entry):0;}
 }
