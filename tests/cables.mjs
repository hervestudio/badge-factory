...
const cables=createCables(root,parts);assert.equal(cables.nets.filter(n=>!n.disconnected).length,24);assert.equal(cables.nets.length,25);
...
assert.ok(cables.nets.find(n=>n.name==='Display / SDO').disconnected);
for(const progress of [0,.5,1,6.9,7.3,7.4,8,9,10]){
 ...
 if(progress>=7.4)assert.ok(cables.nets.every(n=>n.shown===1));
...
console.log('24 connections + trimmed SDO verified: solder endpoints remain pinned during opening and closing; all vertices finite; SPI, power and unconnected SDO mappings checked.');
